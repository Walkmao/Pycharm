...
if __name__ == '__main__':
    ...
    # 连接数据库
    host_l = ['10.45.0.138','10.45.0.139']
    user_ = 'sa'
    password_ = 'sql2k8@123'
    databases = ['PT_SWAP']
    ...
    for database in databases:
        ...
        conn = pymssql.connect(host=host_l[0], port=1433, user=user_, password=password_, database=database)
        cursor = conn.cursor()

        print("连接到数据库：", database)

        # 读取Excel表
        path = r'C:\Users\Mirco\Desktop\table_list2.xlsx'
        table_list = Read_list(path)

        table_list2 = table_list['table_name']
