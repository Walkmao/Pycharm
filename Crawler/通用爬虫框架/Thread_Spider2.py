...
class Spider(object):
    ...
    def producer(self, urllists):
        # 从每页网址中爬取目标网址并加入到集合new_urls
        for url in urllists:
    ...
    def main(self):
        root_url = "https://su.lianjia.com/ershoufang/pg{}"
        ...
        threads = []
        for i in range(2):
            t1 = threading.Thread(target=self.producer(urllists))
            threads.append(t1)

        for i in range(5):
            t2 = threading.Thread(target=self.customer())
            threads.append(t2)
